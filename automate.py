...
async def plot_gwaff(interaction: discord.Interaction,
                     days: app_commands.Range[float, 1, 100] = 7,
                     count: int = 15,
                     hidden: bool = True):
...
async def plot_daily(interaction: discord.Interaction,
                     hidden: bool = False):
    await interaction.response.defer(ephemeral=hidden)
    growth(days=1, special=True)
    await interaction.followup.send(file=discord.File('out.png'))
...
async def last_record(interaction: discord.Interaction,
                      hidden: bool = True):
    await interaction.response.defer(ephemeral=hidden)
    now = datetime.now()
    data = pd.read_csv("gwaff.csv", index_col=0)
    ...
    unixtime = time.mktime(last.timetuple())
    alive = "" if (now - last).total_seconds() < 1.1 * \
        60*60 else "Collection has halted!"
    await interaction.followup.send('Data was last collected <t:'+str(round(unixtime))+':R> \n'+alive)
...
async def plot_growth(interaction: discord.Interaction,
    ...
    try:
        ...
        await interaction.followup.send("That person has not been online recently enough")
        return
    await interaction.followup.send(file=discord.File('out.png'))
...
async def rank_true(interaction: discord.Interaction,
    ...
    index, xp, other, other_xp = rank.find_index(member.id)

    member_name = "You are" if member == interaction.user else "<@" + \
        str(member.id)+"> is"

    if other_xp-xp <= 0:
        await interaction.followup.send(member_name+" ranked "+ordinal(index+1)+" in the server")
    else:
        await interaction.followup.send(member_name+" ranked "+ordinal(index+1)+" in the server, "
                                        + str(round(other_xp-xp))+" xp behind <@"+str(other)+">")
...
async def leaderboard(interaction: discord.Interaction,
    ...
    if len(description) <= 0:
        ...
        return
    description += "\nPage: "+str(page)+"/"+str(ceil(len(rank.values)/25))
    board = discord.Embed(title='Leaderboard', description=description, colour=discord.Colour.from_str('#ea625e'))
...
async def set_time_offset(interaction: discord.Interaction,
    ...
    with open('offsets.json', 'w') as f:
        json.dump(data,f)
    await interaction.followup.send("Successfully set your timezone to be UTC"+"+"*(offset>=0)+str(offset))
